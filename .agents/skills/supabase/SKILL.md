...
## Twinely DEV Edge Function Deploys

In `/Users/drijka/DevProjects/twinely`, DEV is Supabase project `pdrvjjijsmmxtyngoplc`.

When deploying Edge Functions for local Twinely development, do not rely on an already-exported `SUPABASE_ACCESS_TOKEN`; it can be stale or point to a PAT without management permissions. Load and export the repo `.env` first, then deploy and verify:

```bash
set -a
source .env
set +a
supabase functions deploy <function-name> --project-ref pdrvjjijsmmxtyngoplc --use-api
supabase functions list --project-ref pdrvjjijsmmxtyngoplc
```

For a single prompt/runtime fix, deploy only the changed function, for example:

```bash
set -a
source .env
set +a
supabase functions deploy process-os-jobs --project-ref pdrvjjijsmmxtyngoplc --use-api
supabase functions list --project-ref pdrvjjijsmmxtyngoplc | rg "process-os-jobs"
```

If a direct CLI deploy returns management `403`, retry once with the `.env` PAT exported as above before reporting a permission blocker. Do not print the token value.
