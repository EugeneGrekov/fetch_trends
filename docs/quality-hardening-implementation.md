...
- `scripts/check-command-docs.ts` adds an offline check for README, optional `docs/commands.md`, and recipe `npm run` references against `package.json`.
...
- The command-documentation checker validates `docs/commands.md` only when that file exists, so it can coexist with checkouts before the release command reference is committed.
