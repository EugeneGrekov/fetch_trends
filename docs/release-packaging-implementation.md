...
- Updated `docs/architecture.md` with the release packaging boundary.
...
- `docs/commands.md` is tested against all current `package.json` scripts.
...
- Re-run `npm run release:check` after future command additions so `docs/commands.md` stays aligned with `package.json`.
